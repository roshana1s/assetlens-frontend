...
const ReportDashboard = () => {
  ...
  const fetchReports = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${API_BASE_URL}/reports?org_id=1&user_id=u0002&limit=50`);
  ...
  const downloadReport = async (reportId) => {
    try {
      const response = await fetch(`${API_BASE_URL}/reports/${reportId}?org_id=1`);
  ...
  const deleteReport = async (reportId) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/reports/${reportId}?org_id=1&user_id=u0002`, {
        method: 'DELETE'
      });
